...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        ## YOUR CODE HERE ####
        def parents_list(node):
            plist = []
            while node.get_parent():
                plist.append(node.get_parent().name)
        ...
        if a_node == b_node:
            ...
            a_parents_list = parents_list(a_node)
            b_parents_list = parents_list(b_node)
            if a in b_parents_list:
                t = -1
                r = b_parents_list.index(a) + 1
            elif b in a_parents_list:
                t = -1
                r = a_parents_list.index(b) + 1
            else:
                a_b_sameparents = list(set(a_parents_list).intersection(set(b_parents_list)))
                a_left = list(set(a_parents_list).difference(set(a_b_sameparents)))
                b_left = list(set(b_parents_list).difference(set(a_b_sameparents)))
                t = min(len(a_left), len(b_left))
                r = abs(len(a_left) - len(b_left))
        return t, r
...
t, r = f.cousin("f", "g")
print "'f' is a", words[t], "cousin", r, "removed from 'g'"
